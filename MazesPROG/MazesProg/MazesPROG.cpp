...
void SameCellRobots(vector<string>& robots, const int robotsX, const int robotsY) {
...
void GameOver(const vector<vector<char>>& maze, string playerPos, unsigned mazeNum) {
    ...
    if (playerPos != "h")
    {
        cout << "Congratulations! You survived the maze! " << endl;
        cout << "Time Player " << time << " s" << endl;
        ...
        int num_spaces = 20 - playerName.size(); // 
        string spaces(num_spaces, ' ');    //  fill name with " " untill desired length ( so it fits the leaderboard )
        playerName = playerName + spaces;              //

        double timeRounded = round(time * 1000.0) / 1000.0; // rounding time to 3 digits
        string stringRounded = to_string(timeRounded);
        for (int i = stringRounded.size() - 1; i > 0; i--) // even after rounding, a few of the 0s to the right are considered in the .size() function, this for loop is a work-around.
        {
        ...
        int time_size = stringRounded.size();

        num_spaces = 0;                                 // 
        num_spaces = 6 - time_size;                     // num of " " to get desired length 
        string time_spaces(num_spaces, ' ');			//

        string leaderString = playerName + "-" + time_spaces + stringRounded;
...
void PlayerInput(vector<vector<char>>& maze, string& playerPos, vector<string>& robots) {
...
void MoveRobots(vector<vector<char>>& maze, string& playerPos, vector<string>& robots, unsigned mazeNum) {
    ...
    if (playerPos != "h")
    {
        ...
        for (size_t i = 1; i < robots.size(); i++)
        {
            ...
            if (robots[i] == "r") {
                ...
                space = robots[i].find(" ");
                robotX = stoi(robots[i].substr(0, space)); //get the position of the robot[i]
    ...
    if (playerPos == "h" || !RobotsAlive(robots)) //in case robots killed the player or robots died
    {
